...
[English / 詳細な設定手順](README.md) · [リリース](https://github.com/TheNINJALLO/endstone-inventory-share-plugin/releases/tag/v2.7.6)

## ログイン時の切断修正

v2.7.6 は一時的なデータベース障害やサーバー移動時の保存待ちを、既定で15秒間再試行します。経験値やタグの復元だけが失敗した場合は、該当する保存データを保持したままインベントリを復元します。読み込み中はインベントリ操作を制限します。

`INV-LEGACY` は旧バージョンのログインロックを示します。対象プレイヤーが全サーバーから退出したことを確認してから、サーバーコンソールで `invshare recoverlegacy 対象の数値XUID confirm-offline` を実行してください。この操作はインベントリを削除せず、新しいセッショントークンを持つロックには適用されません。詳細は [復旧ガイド](docs/recovery.md) を参照してください。
...
2. 同じデータベースを使用する全サーバーを停止し、全ての古い wheel を v2.7.6 に置き換えます。旧バージョンとの混在運用はしないでください。
